...
Line 120:2 Transforming let to the configured target environment ("es5") is not supported yet
Line 123:8 Transforming let to the configured target environment ("es5") is not supported yet
Line 142:2 Transforming let to the configured target environment ("es5") is not supported yet
Line 143:2 Transforming let to the configured target environment ("es5") is not supported yet
Line 146:8 Transforming let to the configured target environment ("es5") is not supported yet
Line 147:8 Transforming let to the configured target environment ("es5") is not supported yet
Line 150:10 Transforming const to the configured target environment ("es5") is not supported yet
Line 185:4 Transforming let to the configured target environment ("es5") is not supported yet
Line 187:8 Transforming let to the configured target environment ("es5") is not supported yet
Line 434:1988 Transforming const to the configured target environment ("es5") is not supported yet
  ...
  function animateObserver(){
    ...
    let lazyAnimateObserver = new IntersectionObserver(function(entries, observer) {
      entries.forEach(function(entry) {
        ...
        var aaS = lazyAnimate.querySelector('.animate-section');
        if (aaS) {
          if (entry.isIntersecting) {
  ...
  sU.forEach(function(s) {
    s.onclick = function(ev){
      ...
      document.querySelectorAll('[name]').forEach((item, i) => {
        if (item.getAttribute('name').includes('properties')) {
          let property = item.getAttribute('name').toString();
          let propName = property.match(/\[(.*?)\]/)[1];
          let propValue = item.value;
          if (item.type === 'checkbox' && item.checked || item.type === 'radio' && item.checked) {
            properties[propName] = propValue;
          } else if (item.type !== 'checkbox' && item.type !== 'radio' && propValue !== '') {
            properties[propName] = propValue;
          }              
        }
      });
      if (p.elements['selling_plan']) {
        var sP = p.elements['selling_plan'].value;
      } else {
        var sP = '';
      }
      ...
      fetch(theme.routes_cart_add_url + '.js', {
        ...
        if (j.status == 422) {          
          ...
          s.innerText = theme.language.products_product_sold_out;
        } else {
          ...
          window.setTimeout(function(){
            ...
            s.innerText = theme.language.products_product_add_to_cart
          }, 2000);
    ...
    if (j.items.length > 0) {
      ...
      if (j.items.length > 1) {
        ...
        if (theme.dropdown == true) {
          ...
          document.querySelector('.cart-count-mobile').classList.add('void');
        }
      };
      if (j.items.length > 2) {
      ...
      for (var i = 0;  i < q; i++) {
        ...
        li.classList.add('cart-product','relative','clearfix');
        ...
        if (m) {
          ...
          img.classList.add('lazyload','lazyloaded','img-align','dropdown');
        ...
        if (j.items[i].selling_plan_allocation) {
          sp.classList.add('selling');
          sp.textContent = j.items[i].selling_plan_allocation.selling_plan.name;
          it.appendChild(sp);
          d.appendChild(it);
          tc.appendChild(d);
        }        
...
function LiveSearch(container) {
  ...
  var searchForms=el.forEach(function(e) {
    ...
    const btn = document.createElement('a');
    btn.classList.add('modal-close','hidden');
    btn.setAttribute('href', 'javascript: void(0)')
          ...
          function f(t) {
            if (t) {
              ...
              if (t.results_count > 10) {
                ...
                li.classList.add('see-all','product','twelve','text-center') 
