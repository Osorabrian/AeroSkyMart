...
Line 464:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 763:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 764:4 Transforming let to the configured target environment ("es5") is not supported yet
Line 767:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 768:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 772:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 773:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 774:4 Transforming const to the configured target environment ("es5") is not supported yet
Line 779:6 Transforming const to the configured target environment ("es5") is not supported yet
Line 780:6 Transforming const to the configured target environment ("es5") is not supported yet
...
function ProductOptionsJS(container){
  ...
  var srfc = container.querySelector('.product-single__store-availability-container');
  ...
  Shopify.linkOptionSelectors = function(product) {
    ...
    document.querySelector(selectors.sos1).addEventListener('change', function(){
      if (product.options.length === 3 && product.options.available) Shopify.updateOptionsInSelector(2);
      return true;
    });
  };
...
function ProductSelectCallback(container) {
  ...
  var selectCallback = function(variant, selector) {    
    ...
    if (variant && variant.available == true) {
      ...
      var label = container.querySelectorAll('.selectors label span');
      if (swatch) {
    ...
    if (variant) {
      ...
      for (var i=0,length=variant.options.length; i<length; i++) {
        ...
        var label = form.querySelector('.selectors[data-option-index="' + i + '"] label span');
